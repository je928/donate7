...
<script type="text/javascript">
				...
				$('#vt_class').change(
						function() {
							var sendData = 'class_no='
									+ $('#vt_class option:selected').val()
									+ '&dclass_no=0&type=S';
							if ($('#vt_class option:selected').val() != 0) {
								$.post('dclassList.do', sendData,
										function(data) {
											$('#res').html(data);
										});
							} else {
	...
	function list(pageNum) {
		...
		var rec_param = {
			...
			"vt_dclass" : $('#vt_dclass').val() != null ? $('#vt_dclass').val()
					: 0,
...
<body>
	...
	<div class="form-area req-container" style="margin-left: 110px;">
		...
		<div class="form-group">
			<b>봉사지역: </b> <select id="sido" name="sido_no"
				class="form-control-40" style="display: inline; width: 150;">
				<option value="0">전체</option>
				<c:forEach var="addr" items="${addr}">
					<option value="${addr.sido_no}">${addr.sido_name}</option>
				</c:forEach>
			</select> <span id="gugun"></span>
		</div>
		<div class="form-group">
			<div>
				<b>모집 기간</b> <b>시작일:</b><input class='form-control-20' type='date'
					name='vt_r_start_date' id='rsdate' value='${rec.vt_r_start_date}' />
				<b>종료일:</b><input class='form-control-20' type='date'
					name='vt_r_end_date' id='redate' value='${rec.vt_r_start_date}' />
			</div>
			<div>
				<b>봉사 기간</b> <b>시작일:</b><input class='form-control-20' type='date'
					name='vt_a_start_date' id='asdate' value='${rec.vt_a_start_date}' />
				<b>종료일:</b><input class='form-control-20' type='date'
					name='vt_a_end_date' id='aedate' value='${rec.vt_a_start_date}' />
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			<div style="float: left; width: 50%">
				<b>봉사 제목:</b> <input type="text" class="form-control-50"
					id="vt_name" name="vt_name" maxlength="100" required
					style="display: block;">
			</div>
			<div>
				<b>모집 상태:</b> <select id="vt_r_yn" name="vt_r_yn"
					style="display: block;">
