...
<body>
				...
				<table class="table table-striped table-bordered table-list">
					...
					<c:forEach var="pr" items="${list}">
						<tr>
							<td>${num}</td>
							<td><a
								href="o_prView.do?pr_no=${pr.pr_no}&pageNum=${pageNum}">
									${pr.pr_proname }</a></td>
							<td>${pr.pr_qty}</td>
							<td>${pr.pr_price}</td>
							<td>${pr.pr_date}</td>
							<td><c:if test="${pr.pr_approve eq 'y'}">
								...
								</c:if> <c:if test="${pr.pr_approve eq 'n'}">
	...
	<div align="center">
		<button class="btn btn-sm btn-success"
			onclick="location.href='o_prWrite.do'">
			요청하기 <i class="glyphicon glyphicon-envelope"></i>
		</button>
	</div>
