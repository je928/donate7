...
public class SecondController {
	...
	public String second(@RequestParam("image") MultipartFile mf, HttpServletRequest request, Model model,
			Second second, HttpSession session) throws IllegalStateException, IOException {
	...
	public String msecondupdate(@RequestParam("image") MultipartFile mf, HttpServletRequest request, Model model,
			Second second, HttpSession session) throws IllegalStateException, IOException {
		int no = (Integer) session.getAttribute("no");
	...
	public String osecond(@RequestParam("image") MultipartFile mf, HttpServletRequest request, Model model,
			Second second, HttpSession session) throws IllegalStateException, IOException {
		String fileName = mf.getOriginalFilename();
		String uploadName = System.currentTimeMillis() + mf.getOriginalFilename();
		mf.transferTo(new File(request.getRealPath("/") + uploadName));
	...
	public String osecondupdate(@RequestParam("image") MultipartFile mf, HttpServletRequest request, Model model,
			Second second, HttpSession session) throws IllegalStateException, IOException {
		int no = (Integer) session.getAttribute("no");
	...
	/*
	 * @RequestMapping(value = "admemList", method = RequestMethod.GET) public
	 * String admemList(Second second, String pageNum, HttpSession session,Model
	 * model) { int no = (Integer)session.getAttribute("no"); if(pageNum== null
	 * || pageNum.equals("")){ pageNum="1"; }
	 * 
	 * second.setSh_mno(no); int nowPage = Integer.parseInt(pageNum); int total
	 * = ss.memTotal(second); Paging pg = new Paging(nowPage, total);
	 * second.setStartRow(pg.getStartRow()); second.setEndRow(pg.getEndRow());
	 * List<Second> memlist = ss.admlist(second);
	 * 
	 * model.addAttribute("memlist", memlist); model.addAttribute("total",
	 * total); model.addAttribute("pg", pg); model.addAttribute("pgm",
	 * "../member/admin_page/a_tamp.jsp"); model.addAttribute("mypgm",
	 * "../../second/adsecond/admemList.jsp"); return "module/main"; }
	 */
